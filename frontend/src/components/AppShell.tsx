...
export function AppShell({ page, children }: { page: string; children: ReactNode }) {
  ...
  return <div className="shell">
    ...
    <aside className="sidebar">
      <a href="#/overview" className="brand" onClick={() => setOpen(false)}><span className="brand-icon"><TrainFront size={25} /></span><span>NEBULA<span className="brand-x">X</span><small>ENGINEERING SYSTEMS</small></span></a>
    ...
    <div className="workspace"><header className="topbar"><span>Train Condition Monitoring</span><span className="mode"><span className="small-indicator" />{mockMode ? 'Development fixtures' : 'Backend API mode'}</span></header>
      ...
      <main id="main" tabIndex={-1}>{children}</main><footer>NEBULAX / PS3 <span>Model findings support engineering review.</span></footer>
    </div>
  </div>;
}
