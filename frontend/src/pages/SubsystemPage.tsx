...
import { config, uploadMetadata } from '../utils/domain';
...
export function SubsystemPage({ subsystem, runId }: { subsystem: Subsystem; runId?: string }) {
  ...
  const metadata = runId ? uploadMetadata(runId) : undefined;
  return <><div className="page-heading"><p className="eyebrow">SUBSYSTEM ANALYSIS</p><h1>{config[subsystem].name}</h1><p>{config[subsystem].description}</p></div>
    ...
    {run && <div className="run-meta"><span>Run <code>{run.run_id}</code>{metadata ? ` · ${metadata.filenames.join(', ')}` : ''}</span><StatusBadge value={run.status} /></div>}
